...
5. [Empty `ports: []` Silently Omits PublishPort — Readiness Probe on Localhost Will Always Fail](#5-empty-ports--silently-omits-publishport--readiness-probe-on-localhost-will-always-fail)
6. [Stale Quadlet After Config Change Requires Full Stop → Delete → Regen — Restart Is Not Enough](#6-stale-quadlet-after-config-change-requires-full-stop--delete--regen--restart-is-not-enough)
...
# 5 Empty `ports: []` Silently Omits PublishPort — Readiness Probe on Localhost Will Always Fail

**Version:** Podman 5.8.1, quadlet  
**Discovered:** 2026-03-21, Phase 9c SOL deployment

## What Happened
`setup-worker.sh` started the ollama container and then waited up to 120 s for `curl http://localhost:11434/` to succeed. The curl timed out, the script errored, and the journal showed ollama running fine and listening inside the container with `Listening on [::]:11434 (version 0.18.2)`.

On investigation: `podman ps --format "{{.Ports}}"` returned nothing — the container had no host port binding. `curl http://localhost:11434/` reached nothing, even though the service was healthy inside the pod network.

## Root Cause
`config.json` had `"ports": []` for the ollama service. The quadlet generator emitted no `PublishPort=` directive. The container was accessible from other containers on `ai-stack-net` (e.g. LiteLLM via `http://ollama.ai-stack:11434`) but not from the host loopback interface.

The readiness probe and the controller's remote access both depend on the host port binding. Neither worked.

## Fix
Add the port binding in `config.json`:
```json
"ports": [{"host": 11434, "container": 11434, "bind": "0.0.0.0"}]
```
Regenerate the quadlet and restart the service. Verify with:
```bash
grep PublishPort ~/.config/containers/systemd/ollama.container
podman ps --format "{{.Names}} {{.Ports}}"
```

## Rule
> After generating quadlets for any service that needs host-reachable ports, always verify `PublishPort=` appears in the `.container` file **before** starting the service. A readiness probe that polls `localhost` will silently loop to timeout if the port is not published — the service may be perfectly healthy inside the container.
...
## What Happened
After fixing `config.json` (adding `PublishPort`, removing `CUDA_VISIBLE_DEVICES`, enabling CDI GPU passthrough), the ollama container was restarted. The journal still showed `CUDA_VISIBLE_DEVICES=0` override warnings and the port remained unpublished. The fixes had no effect.

## Root Cause
Quadlet `.container` files are regenerated by the quadlet generator only when `systemctl --user daemon-reload` is called. Simply restarting the service reuses the existing generated unit in `/run/user/<uid>/systemd/generator/`, which was built from the old `.container` file. If the `.container` file in `~/.config/containers/systemd/` has also changed, the generator will rebuild it on the next `daemon-reload`.

However, if the `.container` file had **not** been replaced (e.g. quadlets were not regenerated via `configure.sh`), the stale file stays in place and `daemon-reload` still produces the old unit.

## Fix
Full cleanup sequence when config changes affect an already-deployed service:
```bash
systemctl --user stop ollama.service
rm ~/.config/containers/systemd/ollama.container
# Regenerate from updated config
bash scripts/configure.sh generate-quadlets   # or setup-worker.sh
systemctl --user daemon-reload
systemctl --user start ollama.service
```

## Rule
> After any change to `config.json` that affects a container's quadlet (ports, env, devices, image tag), always: **stop** the service, **delete** the old `.container` file, **regenerate** via `configure.sh`, **daemon-reload**, then **start**. Never skip the delete step — the generator will not overwrite a file that still exists with unchanged mtime.
