...
# 16 Current-State Package Map

> Added as part of the 2026-07-29 governance retrofit. This section reflects what is actually
> on disk today; §1–15 above describe the original design and may lag deployed reality
> (e.g. §14's tracker shows items "Not started" that are in fact complete — see
> `docs/wip/plan.md` and `docs/decisions.md` for current status). When this section and the
> body of the document disagree, this section wins for orientation purposes; `docs/decisions.md`
> wins for the historical record of *why*.

**Config map** — every `configs/<dir>` maps to one deployed concern:

| Directory | Concern | Notes |
|---|---|---|
| `configs/quadlets/` | systemd quadlet unit files, one per service + the heartbeat timer | Generated by `scripts/configure.sh generate-quadlets` — do not hand-edit, regenerate from `config.json` |
| `configs/reverse-proxy/caddy/` | Public-facing edge (Caddyfile) — photondatum.space, mail/git subdomains | Current edge for external routing; nginx was removed in favor of Caddy (see `docs/decisions.md`) |
...
**Composition rule:** all service definitions originate in `configs/config.json` (single source of truth) and are compiled to quadlets via `scripts/configure.sh generate-quadlets`. Never hand-edit a generated quadlet — edit `config.json` and regenerate.
