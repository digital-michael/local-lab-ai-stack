...
| # | Improvement | Triggered By |
|---|---|---|
| I-9 | **Hardware-first: always run `nvidia-smi` before writing GPU config** — vLLM guidance was authored assuming a 24 GB GPU node; `nvidia-smi` run late in the project revealed RTX 3070 Ti (8 GB VRAM) instead. Four specific sub-lessons: (1) FP16 model sizing: an 8B-parameter model at FP16 requires ~16 GB VRAM — only quantized variants (AWQ, GPTQ, Q4) fit in 8 GB; (2) desktop GPU overhead: ~2–3 GB of VRAM is consumed by X server and driver reservation on a live desktop; set `GPU_MEMORY_UTILIZATION=0.60` not 0.90; (3) multi-service GPU conflict: Ollama will claim the GPU at container startup unless explicitly pinned to CPU via `CUDA_VISIBLE_DEVICES=""`; (4) detect before configure: `configure.sh detect-hardware` must precede any GPU service configuration on a new node | W-9 — vLLM config written for imaginary hardware; discovered late via `nvidia-smi` |
| I-10 | **Always use fully-qualified image names in config.json** — Podman's short-name resolution requires interactive TTY to resolve ambiguous registries; systemd units never have a TTY, so any image without a `docker.io/` (or other registry) prefix fails immediately with `short-name resolution enforced but cannot prompt without a TTY`. Rule: every image in config.json must start with a registry prefix. Also confirmed: vLLM does use v0.17.x versioning (not v0.6.x) — always verify real tag names before writing config rather than guessing from memory | W-10 — vLLM service failed at first start; root cause was `vllm/vllm-openai` short name |
