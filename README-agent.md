...
This repository is enrolled in the LLM Agent Collaboration Framework (domain: `photon-datum`). See §13 for the session protocol and where framework governance lives.
...
| Architecture decision records (ADRs) | [docs/decisions.md](docs/decisions.md) — ⚠ see Known Open Issue below |
| Agent governance enrollment, lessons index | [docs/governance/README.md](docs/governance/README.md) |
...
**⚠ Known Open Issue:** `docs/decisions.md` and `docs/meta_local/decisions.md` are two independently-numbered ADR logs that have diverged (both have a "D-001" with different content). Do not treat `docs/meta_local/decisions.md` as authoritative until this is reconciled by the operator. See `docs/governance/README.md` §Open Issues.
...
├── .llm-framework.yml                   # Framework enrollment config
├── configs/
...
└── docs/
    ...
    ├── governance/                      # Framework enrollment, lessons index, assignment/session-context
...
Session protocol is governed by the LLM Agent Collaboration Framework:

- **Protocol definition:** `~/Documents/Entities/frameworks/llm-agent-framework/governance/agent-context-protocol.md`
- **Domain profile:** `~/Documents/Entities/frameworks/llm-agent-domains/photon-datum/`
- **Per-repo session-start load order:** `~/Documents/Entities/frameworks/llm-agent-domains/photon-datum/local-lab-ai-stack/README.md`
- **Project governance enrollment index:** `docs/governance/README.md`
- **Forcing function:** `.github/copilot-instructions.md` (auto-injected by VS Code Copilot)

Every agent session in this repo must establish **Locked-In** status before proceeding: governance read, lessons-learned scanned, work tracking active. Default profile for this repo: `standard`.

This repo is worked from multiple workstations (CENTAURI, TC25, and others as added). The framework repos above must be cloned to the identical path relative to `$HOME` on every workstation — see `docs/governance/README.md` §Multi-Workstation Convention.
